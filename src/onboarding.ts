/**
 * Museve Voice setupWizard —— 声明式 channel setup wizard。
 *
 ...
 */
export const museveVoiceSetupAdapter: any = {
  applyAccountConfig: ({ cfg, input }: any) => {
    if (input?.token !== undefined) {
      setMuseveAccount(cfg, { _pendingCode: String(input.token).trim().toUpperCase() });
    }
    return cfg;
  },
};
 ...
 */
export const museveVoiceSetupWizard: any = {
  channel: CHANNEL_ID,
  ...
  status: {
    ...
    resolveConfigured: ({ cfg }: any) => hasCompleteMuseveBinding(resolveMuseveAccount(cfg)),
    resolveStatusLines: ({ cfg, configured }: any) => {
      const account = resolveMuseveAccount(cfg);
      if (configured) {
        return [
          `Museve Voice: 已绑定到 ${readNonEmptyString(account, "boundUserName") || readNonEmptyString(account, "boundUserId") || "(未知)"}`,
        ];
      }

      const missing = missingMuseveBindingFields(account);
      return [
        missing.length > 0
  ...
  // ── 引导说明（首次进入向导时显示） ──────────────────────────────────
  introNote: {
    title: "Museve Voice 绑定",
    lines: [
      "请在 Museve App 点击「连接 OpenClaw」获取 8 位绑定码。",
      "绑定码 5 分钟内有效；累计失败 ≥5 次会作废，需在 App 重新生成。",
    ],
    shouldShow: ({ cfg }: any) => !hasCompleteMuseveBinding(resolveMuseveAccount(cfg)),
  },
  ...
  credentials: [
    {
      ...
      inspect: ({ cfg }: any) => {
        ...
        const configured = hasCompleteMuseveBinding(account);
        return {
      ...
      applySet: ({ cfg, resolvedValue }: any) => {
        ...
        setMuseveAccount(cfg, { _pendingCode: String(resolvedValue).trim().toUpperCase() });
        return cfg;
      },
    },
  ],
  ...
  finalize: async ({ cfg }: any): Promise<{ cfg: any } | undefined> => {
    ...
    if (!CODE_REGEX.test(pendingCode)) {
      setMuseveAccount(cfg, { _pendingCode: "" });
  ...
  // ── 禁用 ───────────────────────────────────────────────────────────
  disable: (cfg: any) => {
    setMuseveAccount(cfg, { enabled: false });
    return cfg;
  },
};
